...
export class LatticeWebsite extends Construct implements LatticeWebsiteConstruct {
  ...
  constructor(scope: Construct, id: string, props: LatticeWebsiteProps) {
    ...
    const { name, environment, sourcePath, domainName, errorPage = 'index.html' } = props;
    ...
    if (domainName) {
      ...
      console.log(
        `⚠️ Custom domain ${domainName} specified but not implemented. Using CloudFront domain.`
      );
