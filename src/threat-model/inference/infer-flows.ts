...
export function inferDataFlows(facts: ThreatFactsCollector): DataFlow[] {
  ...
  if (services.has("bedrock") || services.has("sagemaker")) {
    const aiResources = facts.inventory.filter(r => 
      r.service === "bedrock" || r.service === "sagemaker"
    );
    const vectorStores = facts.inventory.filter(r => 
      r.service === "opensearch" || r.service === "pinecone"
    );
