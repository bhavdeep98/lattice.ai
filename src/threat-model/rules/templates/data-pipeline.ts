...
export function dataPipelineThreats(ctx: {
  ...
  threats.push({
    ...
    scenario:
      'A producer impersonates a trusted data source and writes malicious or incorrect events/objects.',
  ...
  threats.push({
    ...
    scenario:
      'An attacker modifies objects or intermediate outputs to poison analytics/ML results.',
  ...
  threats.push({
    ...
    scenario:
      'Processing jobs log sensitive data values, exposing them in CloudWatch Logs or job outputs.',
  ...
  threats.push({
    ...
    scenario:
      'Malicious or malformed input data causes processing jobs to consume excessive resources or fail.',
  ...
  if (ctx.hasRedshift) {
    threats.push({
      ...
      scenario:
        'Processing roles have excessive permissions to data warehouse, enabling unauthorized data access.',
