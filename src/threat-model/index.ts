...
function generateSecurityChecklist(collector: ThreatFactsCollector) {
  ...
  checklist.push({
    ...
    details: unencryptedStores.length > 0 ? `${unencryptedStores.length} stores without encryption` : undefined
  });
  ...
  checklist.push({
    ...
    details: publicEndpoints.length > 0 ? `${publicEndpoints.length} public endpoints found` : undefined
  });
...
function generateOpenQuestions(collector: ThreatFactsCollector, workloadType: string): string[] {
  ...
  if (collector.dataStores.length > 0) {
    questions.push("What types of sensitive data (PII, PHI, financial) will be stored in this system?");
  ...
  if (publicEndpoints.length > 0) {
    questions.push("What authentication and authorization mechanisms will be implemented for public endpoints?");
