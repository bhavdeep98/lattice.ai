...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function generateManifest(config) {
    ...
    manifestOutput.innerHTML =
      '<button class="copy-button" onclick="copyToClipboard(\'manifest\')">Copy</button><pre><code>' +
      manifestCode +
      '</code></pre>';
  }
...
exports.handler = async (event) => {
    ...
    cloudformationOutput.innerHTML =
      '<button class="copy-button" onclick="copyToClipboard(\'cloudformation\')">Copy</button><pre><code>' +
      generatedCFN +
      '</code></pre>';
  ...
  function generateCDKCode(config) {
    ...
    cdkOutput.innerHTML =
      '<button class="copy-button" onclick="copyToClipboard(\'cdk\')">Copy</button><pre><code>' +
      cdkCode +
      '</code></pre>';
  }
  ...
  function generateCostEstimate(config) {
    ...
    if (config.includeDatabase) {
      ...
      costs.push({
        ...
        cost: dbCost,
        note: config.environment === 'prod' ? 'Multi-AZ' : 'Single-AZ',
      });
      total += dbCost;
    }
  ...
  function generateThreatModel(config) {
    ...
    if (config.includeApi) {
      threats.push({
        level: 'warning',
        icon: '⚠️',
        text: 'API Gateway lacks rate limiting - consider adding throttling',
      });
    }
    ...
    if (config.includeDatabase) {
      threats.push({
        level: 'info',
        icon: '✅',
        text: 'Database in private subnet with security groups',
      });
      if (!config.enableBackups) {
        threats.push({
          level: 'warning',
          icon: '⚠️',
          text: 'Database backups disabled - data loss risk',
        });
      }
    }
    ...
    if (config.includeWebsite && !config.domainName) {
      threats.push({
        level: 'warning',
        icon: '⚠️',
        text: 'No custom domain - using CloudFront default domain',
      });
    }
    ...
    if (!config.includeAuth && config.includeApi) {
      threats.push({
        level: 'critical',
        icon: '🔴',
        text: 'API has no authentication - public access risk',
      });
    }
