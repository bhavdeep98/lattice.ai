...
${constructorCode}
  ...
  function generateInfrastructurePreview(intent) {
        ...
        resources = [
          {
            icon: '🪣',
            name: 'S3 Bucket',
            details: `Encrypted, ${environment === 'prod' ? 'Versioned' : 'Basic'}`,
          },
        ...
        resources = [
          {
            icon: '🗄️',
            name: 'RDS Instance',
            details: `${environment === 'prod' ? 'Multi-AZ' : 'Single-AZ'}, Encrypted`,
          },
        ...
        resources = [
          {
            ...
            details: `${environment === 'prod' ? '1024MB' : '512MB'} memory`,
          },
          { icon: '🔐', name: 'IAM Role', details: 'Least-privilege execution role' },
          {
            icon: '📊',
            name: 'CloudWatch Alarms',
            details: 'Errors, Duration, Throttles monitoring',
          },
      ...
      case 'network':
        resources = [
          { icon: '🌐', name: 'VPC', details: '10.0.0.0/16 CIDR block' },
          {
            icon: '🔗',
            name: 'Subnets',
            details: `${environment !== 'dev' ? 'Multi-AZ' : 'Single-AZ'} public/private`,
          },
          { icon: '🚪', name: 'Internet Gateway', details: 'Public internet access' },
          {
            icon: '🔄',
            name: 'NAT Gateway',
            details: `${environment === 'prod' ? '2 (redundant)' : '1 (cost-optimized)'}`,
          },
    ...
    previewContent.innerHTML = resources
      .map(
        (resource) => `
  ...
  function applySyntaxHighlighting() {
    ...
    if (codeElement) {
      ...
      // Keywords
      html = html.replace(
        /\b(import|export|class|constructor|const|from|new|this|super)\b/g,
      ...
      html = html.replace(
        /\b(Lattice\w+|Stack|Construct|StackProps)\b/g,
        '<span style="color: #4ec9b0;">$1</span>'
      );

      // Properties and variables
      html = html.replace(
        /\b(environment|projectName|owner|name|encryption|versioning|lifecycle)\b/g,
